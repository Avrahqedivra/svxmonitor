...
cookieSettingsName = window.location.hostname+"_svxjson_settings"
...
function getFlag(str) {
    // hack, assume it is a callsign if has a digit in pos 1 or 2 and no space
    if (str.indexOf(" ") == -1 && (isNumeric(str.charAt(1)) || isNumeric(str.charAt(2)))) {
        for(let i=0; i < callsigns.length; i++) {
            ...
            for(let g=0; g < callsignGroup.length; g++) {
                ...
                if (p >= start && p <= end) {
                    if (flag64[callsigns[i].code.toLowerCase()] != null)
                        return flag64[callsigns[i].code.toLowerCase()]
